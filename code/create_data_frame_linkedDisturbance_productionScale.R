...
# The script is set up to work for GEDI collection dates from 2019 - 2020.
...
library(future.apply) #parallel compute
...
#GEDI
gediFiles <- list.files(here(sharedData, "gedi_sre", "gedi_sre"),
                        pattern = "*.shp$",
                        full.names = TRUE)
...
#A SINGLE MASSIVE FUNCTION TO CREATE THE DATA FRAME
create.data.frame <- function(gedi) {
  
  # SCRIPTED ANALYSIS ----
  
  # Manipulate GEDI data ----
  ## Get road buffer data ----
  roadBinary <- roads %>%
    terra::extract(gedi) %>%
    select(-ID)
  gedi <- gedi %>%
    cbind(roadBinary) %>%
    mutate(roadBuffer = ifelse(is.na(roadBuffer, 0, 1)))
  ...
  spei_in_x_yr_prior <- function(dats, x) {
    ...
    dats <- dats %>% transmute({{colNm}} := case_when(gediYr == 2022 ~ NA,
                                                      gediYr == 2021 ~ .[[x]],
  ...
  ndviD <- ndvi %>%
    ...
    select(-"ID") %>%
    cbind(gediYr) %>%
    mutate(peakNDVI = case_when(gediYr == 2022 ~ NA,
                                gediYr == 2021 ~ NDVI2021,
  ...
  time_since_last <- function(dats, distNm) {
    ...
    yrsS <- dats %>% transmute({{colNm1}} := case_when(gediYr == 2022 ~ case_when(rowSums(across(`forest-disturbance-s-rockies-2020`:`forest-disturbance-s-rockies-1999`))==0 ~ NA,
                                                       ...
                                                       gediYr == 2021 ~ case_when(rowSums(across(`forest-disturbance-s-rockies-2020`:`forest-disturbance-s-rockies-1999`))==0 ~ NA,
                                                                                  rowSums(across(`forest-disturbance-s-rockies-2020`:`forest-disturbance-s-rockies-1999`))!=0 ~ 
                                                                                    max.col(across(`forest-disturbance-s-rockies-2020`:`forest-disturbance-s-rockies-1999`), ties.method = "first")),
    ...
    cYD <- dats %>% transmute({{colNm2}} := case_when(gediYr == 2022 ~ NA,
                                                      gediYr == 2021 ~ NA,
  ...
  years_in_prior <- function(dats, distNm, xYrs) {
    ...
    dats <- dats %>% transmute({{colNm}} := case_when(gediYr == 2022 ~ rowSums(across(1:(xYrs-1))),
                                                      gediYr == 2021 ~ rowSums(across(1:(xYrs))),
...
# 
# #set.seed(1)
# gedi_subset_files <- gediFiles[1:30] #working on 1-30
# #gedi_subset_files <- sample(gediFiles, 50)
#  #gedi_subset <- lapply(gedi_subset_files, FUN = function(x) {sf::st_read(x) %>% st_transform(crs(demDats))})
#  #lapply(gedi_subset, FUN = function(x) {unique(year(x$date))})
# gediFiles <- gedi_subset_files



# t <- sf::st_read('C:/dev/linked_disturbance/data/shared/gedi_sre/gedi_sre/gedi_sre_31.shp')
# tt <- unique(year(t$date))
# any(tt == 2019 | tt == 2020)



tic('future_lapply_all')

#batch the files to make progress while figuring out what is wrong & still leveraging parallel compute
set_size <- 10
num_sets <- ceiling(length(gediFiles) / set_size)

# Iterate through each set
#for (i in 1:num_sets) { # ***** THIS IS THE CORRECT LINE *******
for (i in 5:num_sets) {
    
  tic(paste('Set', i, sep = " ")) #Time
  
  # Calculate the starting and ending indices for the current set
  start_index <- (i - 1) * set_size + 1
  end_index <- min(i * set_size, length(gediFiles))
  
  # Extract the current set
  current_set <- gediFiles[start_index:end_index]
  
  #Perform parallel computing on current_set

  future_lapply(current_set, FUN = function(x) {
    gediD <- sf::st_read(x) %>%
      st_transform(crs(demDats))

    #Get just the file name out of the full path
    setNm <- gsub("C:/dev/linked_disturbance/data/shared/gedi_sre/gedi_sre/", "", x) %>%
      gsub(".shp", "", .)
    print(paste("Operating on ", setNm, sep = ""))

    if (any(unique(year(gediD$date)) == 2019 | unique(year(gediD$date)) == 2020)) {
    #Run massive function on operating set
    df <- gediD %>% create.data.frame()

    dfFile <- paste(dfName, "_set_", setNm, ".csv", sep="")
    print(paste("Writing ", dfFile, sep = ""))
    write_csv(df, here(thisRun, dfFile), append = FALSE)
     }
  })
  
  toc() # ~1hr for set of 10, fairly consistent. ~17 hrs total completion estimate
}


toc() #2 hours for 25 / 173 files


# Bind together all csv files
allDatFramesFiles <- list.files(thisRun,
                        pattern = "*.csv$",
                        full.names = TRUE)

allDatFrames <- lapply(allDatFramesFiles, FUN = read_csv)
bigDatFrame <- bind_rows(allDatFrames)
rm(allDatFrames)
gc()

#Filter out low quality data: we want GEDI sensitivity over .95, quality = 1
bigDatFrameClean <- bigDatFrame %>%
  filter(sensitivit >= 0.95 & l2_quality == 1)
...
## Create metadata ----
columns <- c(colnames(bigDatFrameClean)[1:43],
             "spei...",
             "speiCum...",
             "yrsSince...",
             "collectionYr...",
...
# Set output directory & create if doesn't already exist
combinedOutputs <- here::here(thisRun, "combined_outputs")
dir.create(combinedOutputs)


#Write data
dfFile <- paste(dfName, ".csv", sep="")
write_csv(bigDatFrameClean, here(combinedOutputs, dfFile))

stamp <- format(Sys.time(), "%Y-%m-%d %H:%M:%S %Z")

#Sink metadata to text file
sink(here(combinedOutputs, "metadata.md"))
cat("# Metadata for the Southern Rockies Linked Disturbance GEDI Dataset \n")
cat("This dataset includes GEDI points from 2019-2020 that are over areas of the Southern Rockies that were forested at some point between 2000 and 2020. \n")
cat("Data points within a buffer of Census Bureau road polylines have been removed. \n")
cat("Data points with GEDI sensitivity < 0.95 and with an L2 poor quality flag have been removed. \n")
...
cat("Note that there is a significant lack of data for GEDI points collected in 2022. Watch for NA values and know that disturbance information is skipped for 2021 and 2022. \n")
cat("\n")
cat("## Information \n")
...
#Zip together
zip::zip(zipfile = here(combinedOutputs, paste(dfName, ".zip", sep="")),
         files = c(here(combinedOutputs, dfFile),
                   here(combinedOutputs, "metadata.md")),
         mode = "cherry-pick")
